...
void testtranspose ()
{
	...
	{
		if (!myrank)
			printf ("Incorrect number of processes, must test with 3\n");

		MPI_Finalize ();
		exit (0);
	}

	// Declare test matrix size
	int m = 7;
	...
	// Print the results from each process. Correct order is not guaranteed but this kinda helps.
	for (int i = 0; i < commsize; i++)
	{
